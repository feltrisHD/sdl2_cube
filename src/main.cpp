...
int main(int argc, char* args[]) {
    ...
    if (buttonpress == 1) {
        while (true) {
            ...
            while (!quit) {
                ...
                const Uint32 initialSpawnInterval = 200; 
                ...
                for (auto it = hoverCubes.begin(); it != hoverCubes.end();) {
                 ...
                 SDL_Surface* cubeSurface = IMG_Load("cube.png");
    ...
    SDL_FreeSurface(enemySurface);
            ...
            SDL_Rect playerRect = { static_cast<int>(cube.x), static_cast<int>(cube.y), CUBE_SIZE, CUBE_SIZE };
SDL_RenderCopy(renderer, enemyTexture, nullptr, &playerRect);

                }

                SDL_SetRenderDrawColor(renderer, 0x00, 0x00, 0x00, 0xFF); 
                SDL_Rect mainRect = { static_cast<int>(mainX), static_cast<int>(mainY), CUBE_SIZE, CUBE_SIZE };
SDL_RenderCopy(renderer, cubeTexture, nullptr, &mainRect);
